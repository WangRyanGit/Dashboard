...
<head lang="en">
    ...
    <title>策略界面</title>
    <meta name="description" content="这是一个 adstrategy 页面">
    <meta name="keywords" content="adstrategy">
                ...
                <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
                    <div class="widget am-cf">
                        <div class="widget-head am-cf">
                            <div class="widget-title  am-cf">策略列表 AdStrategy</div>
                        ...
                        <div class="widget-body  am-fr">
                                ...
                                <div class="am-form-group">
                                    <div class="am-btn-toolbar">
                                        <div class="am-btn-group am-btn-group-xs">
                                            <button type="button" class="am-btn am-btn-default am-btn-success" onClick="location.href='<%=basePath%>/strategy/toAddAdStrategy'"><span class="am-icon-plus"></span>新增</button>
                            ...
                            <form id="form" method="get" action="<%=basePath%>/strategy/showAllAdStrategy">
                                <div class="am-u-sm-12 am-u-md-6 am-u-lg-3">
                                ...
                                <div class="am-u-sm-12 am-u-md-6 am-u-lg-2">
                                    <div class="am-input-group am-input-group-sm tpl-form-border-form cl-p">
                                        <input type="text" class="am-form-field " name="adid" placeholder="Adid">
                                    </div>
                                </div>
                                <div class="am-u-sm-12 am-u-md-6 am-u-lg-2">
                                    <div class="am-input-group am-input-group-sm tpl-form-border-form cl-p">
                                        <input type="text" class="am-form-field " name="position" placeholder="Position">
                                    </div>
                                </div>
                                <div class="am-u-sm-12 am-u-md-6 am-u-lg-2">
                                    <div class="am-input-group am-input-group-sm tpl-form-border-form cl-p">
                            ...
                            <div class="am-u-sm-12">
                                <form id="form1" method="post" action="<%=basePath%>/strategy/showAllAdStrategy">
                                        ...
                                        <table width="100%" class="am-table am-table-compact am-table-striped tpl-table-black " id="example-r">
                                            ...
                                            <c:forEach items="${requestScope.pagelist.lists}" var="pg">
                                                ...
                                                <tr class="gradeX">
                                                    ...
                                                    <td>${pg.country}</td>
                                                    <td>
                                                        <c:if test="${pg.adType == 1}"> 原生 </c:if>
                                                        <c:if test="${pg.adType == 2}"> 插屏 </c:if>
                                                        <c:if test="${pg.adType == 3}"> Banner </c:if>
                                                    </td>
                                                    <td>
                                                        <c:if test="${pg.adSource == 1}"> Admob </c:if>
                                                        <c:if test="${pg.adSource == 2}"> Facebook </c:if>
                                                        <c:if test="${pg.adSource == 3}"> AdClony </c:if>
                                                        <c:if test="${pg.adSource == 4}"> Vungel </c:if>
                                                        <c:if test="${pg.adSource == 5}"> Unityads </c:if>
                                                    </td>
                                                    <td>${pg.priority}</td>
                                                    <td>${pg.position}</td>
                                                    <td>
                                                        <c:if test="${pg.adreward == 0}"> 非奖励 </c:if>
                                                        <c:if test="${pg.adreward == 1}"> 奖励 </c:if>
                                                    </td>
                                                    <td>
                                                        <c:if test="${pg.status == 0}"><font color="green"> 正常</font></c:if>
                                                        <c:if test="${pg.status == -1}"><font color="#a9a9a9"> 下线 </font></c:if>
                                                    </td>
                                                    <td>
                                                        <div class="tpl-table-black-operation">
                                                            <a href="<%=basePath%>/strategy/toEditAdStrategy?id=${pg.id}">
                                                                <i class="am-icon-pencil"></i> 编辑
                                                            </a>
                                                            <a href="javascript:void(0)" class="tpl-table-black-operation-del" onClick="deleteId(${pg.id})">
                                                                <i class="am-icon-trash"></i> 删除
                                                            </a>
                                                            <%--<a href="<%=basePath%>/strategy/deleteAdStrategy?id=${pg.id}" class="tpl-table-black-operation-del" onclick="alert('确认删除')">
                                                                <i class="am-icon-trash"></i> 删除
                                                            </a>--%>
                                                        </div>
                                                    </td>
                                ...
                                <div class="am-fr">
                                    <ul class="am-pagination tpl-pagination">
                                        ...
                                        <c:if test="${requestScope.pagelist.currPage != 1}">
                                            <li><a href="${pageContext.request.contextPath }/strategy/showAllAdStrategy?currentPage=1&pkg=${requestScope.pagelist.pkg}&adid=${requestScope.pagelist.adid}&position=${requestScope.pagelist.position}">«</a></li>
                                            <li><a href="${pageContext.request.contextPath }/strategy/showAllAdStrategy?currentPage=${requestScope.pagelist.currPage-1}&pkg=${requestScope.pagelist.pkg}&adid=${requestScope.pagelist.adid}&position=${requestScope.pagelist.position}">‹</a></li>
                                        </c:if>
                                        <c:if test="${requestScope.pagelist.currPage != requestScope.pagelist.totalPage}">
                                            <li><a href="${pageContext.request.contextPath }/strategy/showAllAdStrategy?currentPage=${requestScope.pagelist.currPage+1}&pkg=${requestScope.pagelist.pkg}&adid=${requestScope.pagelist.adid}&position=${requestScope.pagelist.position}">›</a></li>
                                            <li><a href="${pageContext.request.contextPath }/strategy/showAllAdStrategy?currentPage=${requestScope.pagelist.totalPage}&pkg=${requestScope.pagelist.pkg}&adid=${requestScope.pagelist.adid}&position=${requestScope.pagelist.position}">»</a></li>
                                        </c:if>
...
<script type="text/javascript">
    ...
    function batchDelete(){
        if(confirm("真的要删除这批策略吗?")){
            $("#form1").attr("action","<%=basePath%>/strategy/batchDelete");
            $("#form1").submit();
        }
    }
    /*   单个删除   */
    function deleteId(id){
        if(confirm("真的要删除吗?")){
            $("#form1").attr("action","<%=basePath%>/strategy/deleteAdStrategy?id="+id);
            $("#form1").submit();
        }
    }
</script>
